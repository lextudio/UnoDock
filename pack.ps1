Param(
    ...
    [string[]]$Projects = @(
        "src\UnoDock\UnoDock.csproj",
        "src\UnoDock.Themes.VS2010\UnoDock.Themes.VS2010.csproj",
        "src\UnoDock.Themes.VS2013\UnoDock.Themes.VS2013.csproj"
    )
)
...
function Find-MSBuild {
    ...
    throw "MSBuild was not found. Install Visual Studio/MSBuild to pack UnoDock."
}
...
function Test-Package([string]$PackagePath) {
    ...
    try {
        ...
        $expectedDllName = if ($packageFileName.StartsWith("LeXtudio.UnoDock.Themes.VS2010.", [StringComparison]::OrdinalIgnoreCase)) {
            "UnoDock.Themes.VS2010.dll"
        } elseif ($packageFileName.StartsWith("LeXtudio.UnoDock.Themes.VS2013.", [StringComparison]::OrdinalIgnoreCase)) {
            "UnoDock.Themes.VS2013.dll"
        } else {
            "UnoDock.dll"
        }

        $dlls = @(Get-ChildItem -Path (Join-Path $extractRoot "lib") -Recurse -Filter $expectedDllName -ErrorAction SilentlyContinue)
        if ($dlls.Count -eq 0) {
            throw "Package '$PackagePath' does not contain $expectedDllName under lib/."
        }
