@echo off
rem build.windows.bat - Locate MSBuild via vswhere and build UnoDock.slnx
...
set "SOLUTION=%SCRIPT_DIR%UnoDock.slnx"
if not exist "%SOLUTION%" (
	set "SOLUTION=UnoDock.slnx"
	if not exist "%SOLUTION%" (
		echo Solution file UnoDock.slnx not found in script folder or cwd.
		endlocal & exit /b 1
	)
)
