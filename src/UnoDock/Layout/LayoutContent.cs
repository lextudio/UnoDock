	...
	public abstract class LayoutContent : LayoutElement, IXmlSerializable, ILayoutElementForFloatingWindow, IComparable<LayoutContent>, ILayoutPreviousContainer
	{
		...
		public bool IsSelected
		{
			...
			set
			{
				...
				_isSelected = value;
				// Only push the parent index upward when this content becomes SELECTED.
				// Pushing -1 when deselecting causes a re-entrant SelectedContentIndex=-1
				// call before the parent has finished its own setter, which triggers WinUI
				// TabControl to re-select item[0] and produce the double-active-tab bug.
				// The parent manages its own index when deselecting (no action needed here).
				if (_isSelected && Parent is ILayoutContentSelector parentSelector)
